	...
	public LoginFrame() {
		...
		signup.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
			    ...
			    int action = JOptionPane.showConfirmDialog(null, confirm,"Xác nhận mật khẩu",JOptionPane.OK_CANCEL_OPTION);
			    if (action == JOptionPane.OK_OPTION) {
			    	if (String.copyValueOf(confirm.getPassword()).equals(String.copyValueOf(txtPassword.getPassword()))) {
							...
							EventQueue.invokeLater(new Runnable() {
								public void run() {
									try {
										...
										int confirm = JOptionPane.showConfirmDialog(null, "Đăng ký thành công\nChào mừng bạn đến với Zalo", "Đăng ký thành công", JOptionPane.DEFAULT_OPTION);
			    		...
			    		notification.setText("Xác nhận mật khẩu không đúng");
			    	}
			    }
			}
		});
